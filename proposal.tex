...
between curves and modular forms and their computational aspects.
Our goals are to provide a setting
to
\begin{itemize}
...
\item aid in the development of number theorists who are early-career or from underrepresented groups; and
...
visitors to the site about some of the central objects of modern number theory and their
interconnections.
...
theory.  We will also have experts from the \textsf{LMFDB} project in attendance to
aid people who may be contributing to that project for the first
time.  Since the \textsf{LMFDB} uses \textsf{Sage}, an open-source computational system,
...
%:modularity is not known in all cases, given the modular form, the construction of
% the corresponding elliptic curve is not known, etc.
% JV: I commented this out.  The cases where modularity is not known (over
% QQ(sqrt5) are those where the level is squarefull (every prime occurs to exponent >= 2).
% This is certainly an important theoretical point but not such a huge
% deal from the point of view of this workshop.
% The "construction" of the elliptic curve is known whenever modularity is known,
% and it boils down to taking the factor of the Jacobian of a Shimura curve.
% This can even be done explicitly in many cases now.  
...
including over 200000 forms over totally real fields of degree at most $6$ and including
...
% These calculations are significantly more difficult than
% the corresponding work on Hilbert modular forms.
% JV: I don't agree with this as written.  If whoever wrote it would like
% to clarify, cool!  But even then, I don't see why it belongs in this proposal.
...
not a Hilbert modular form, and consequently even the
...
y^2=f(x)
\end{equation}
where $f\in k[x]$ is a polynomial of degree 5 or~6 and $k$ is a 
number field.
Our specific focus is on the recent work of Kedlaya and Sutherland and
their collaborators on the
distribution of Euler factors of the $L$-function of the curve.  

Via
their Jacobians, this work
concerns abelian surfaces; at the workshop we will bring out the 
connections with other objects.  Indeed, it is not the hyperelliptic curve itself
but rather its Jacobian $A$ to which we
can associate an $L$-function.  
...
from the Galois action on the Tate module of~$A$ and how
...
such as Siegel modular forms.  The role of the Sato-Tate group in 
identifying
...
curves in the \textsf{LMFDB}, comparable to the Cremona database of elliptic
...
%are of independent interest.  Stein will work with \textsf{LMFDB} collaborator Voight
...
significant ways to the workshop portion of the conference.  We are
requesting funds to pay for travel, food, and lodging for these
people.  
...
to invite at least eight female mathematicians from that group to
